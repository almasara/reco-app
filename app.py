...
# Load CSV + pesan bantu kalau belum ada
try:
...
with tab3:
    st.caption('Butuh artifacts/cf_model.h5 dan TensorFlow. Kalau tidak ada, tab ini kosong.')
    if st.button('Hitung rekomendasi (CF)'):
        out = get_cf_recs(user_id, places, ratings, cf_model, u2e, p2e, topn)
        if out is not None:
            st.dataframe(out, use_container_width=True)
        else:
            st.info('CF model tidak tersedia atau User ID belum dipetakan. Pakai Hybrid/CBF dulu.')
